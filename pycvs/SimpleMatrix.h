...
public:
	...
	#define DEFOP(OP) \
		SimpleMatrix<int> operator OP (const Tp b) {	\
...
template<class Tp>
void Multiply(std::vector<Tp> &out, const SimpleMatrix<Tp>& x, const std::vector<Tp>& y){
		//multiply a matrix with a vector
	Tp			sum;
	int i,k,nr=x.rows(),ny=(int)y.size();
	out.resize(nr); 

	for (i = 0; i < nr; i++){
		sum = 0;
		for(k = 0; k < ny; k++)
			sum = sum+x.Data(i, k)*y[k]; //multiply
		out[i] = sum ;
	}
}

template<class Tp>
void Multiply(std::vector<Tp> &out, const std::vector<Tp>& x, const SimpleMatrix<Tp>& y){
