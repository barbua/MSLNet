...
void Unique(std::vector<Tp> &out, std::vector<Tp> &in){
	...
	std::vector<Tp>::iterator la=std::unique(out.begin(),out.end());
	out.erase(la,out.end());
}

template <typename T>
void SortIndexes(std::vector<size_t> & idx, const std::vector<T> &v) {
...
template <typename T>
void InvSortIndexes(std::vector<size_t> & idx, const std::vector<T> &v) {
  // initialize original index locations
  idx.resize(v.size());
  std::iota(idx.begin(), idx.end(), 0);
...
#define DEFOP(OP) \
	template<class Tp>\
    std::vector<int> operator OP (std::vector<Tp> &x, const Tp &b) { \
		size_t i,n=x.size();\
...
#define DEFOP(OP) \
	template<class Tp>\
    std::vector<Tp> operator OP (const std::vector<Tp> &x, const Tp &b) { \
		size_t i,n=x.size();\
		std::vector<Tp> out(n);\
        for (i = 0; i < n; i++) \
            out[i]=x[i] OP b; \
...
#define DEFOP(OP) \
	template<class Tp>\
    std::vector<Tp> operator OP (const std::vector<Tp> &x, const std::vector<Tp> &b) { \
		size_t i,n=x.size();\
		std::vector<Tp> out(n);\
        for (i = 0; i < n; i++) \
            out[i]=x[i] OP b[i]; \
...
#define DEFOP(OP) \
	template<class Tp>\
    void OP (std::vector<Tp> &x, const Tp &b) { \
		size_t i,n=x.size();\
        for (i = 0; i < n; i++) \
            x[i]=OP(x[i], b); \
    } 
...
template<class Tp>
std::vector<Tp> GetVals(const std::vector<Tp> &x, const std::vector<int> &idx){
	// get x[idx[i]]
...
template<class Tp>
Tp Percentile(const std::vector<Tp> &v, double perc){
...
template<class Tp>
void FindEqual(std::vector<int> &out,std::vector<Tp> &in, Tp val){
	int i,n=(int)in.size();
	out.clear();
	out.reserve(n);
	for (i = 0; i <n; ++i) {
...
template<class Tp>
void FindGreaterEq(std::vector<int> &out,std::vector<Tp> &in, Tp thr){
	int i,n=(int)in.size();
	out.clear();
	out.reserve(n);
	for (i = 0; i <n; ++i) {
...
template<class Tp>
void GetValues(std::vector<Tp> &out, std::vector<Tp> &in, std::vector<int> &idx){
	int i,n=(int)idx.size();
	out.resize(n);
	for (i = 0; i <n; ++i) {
		out[i]=in[idx[i]];
...
template<class Tp>
struct SizeOrder:public std::binary_function<std::vector<Tp>,std::vector<Tp>, bool> {
	bool operator()(const std::vector<Tp> &a, const std::vector<Tp>  &b) const {
...
template<class Tp>
struct InvSizeOrder:public std::binary_function<std::vector<Tp>,std::vector<Tp>, bool> {
	bool operator()(const std::vector<Tp> &a, const std::vector<Tp>  &b) const {
		if (a.size()>b.size()) return true;
		if (a.size()<b.size()) return false;
		int n=(int)a.size();
