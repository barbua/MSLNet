...
template<class Tp>
Tp Curvature(std::vector<Point<Tp>> &p, int i){
	Point<Tp> dx=(p[i+1]-p[i-1])*(Tp)0.5;
	Point<Tp> ddx=(p[i+2]+p[i-2])*(Tp)0.25f-p[i]*(Tp)0.5f;
	Tp num   = dx._x * ddx._y - ddx._x * dx._y;
	Tp denom = dx._x*dx._x + dx._y * dx._y;
	denom = pow(denom, (Tp)1.5);
	return num / denom;
}
...
template<class Tp>
void Round(std::vector<Point<int>> &out, std::vector<Point<Tp>> &in){
	int i,n=(int)in.size();
	out.resize(n);
	for (i=0;i<n;i++)
		out[i].Set((int)(in[i]._x+0.5),(int)(in[i]._y+0.5));
}
...
template<class Tp>
double FindDistEndp(int &i1, int &i2, std::vector<Tp> &c1, std::vector<Tp> &c2){
	// find the smallest distance between the endpoints of curves c1 c2 and return 0/1 for which endpoint it is for each
	int n1=(int)c1.size(),n2=(int)c2.size();
	vector<Tp> v1(2),v2(2);
	v1[0]=c1[0];v1[1]=c1[n1-1];
	v2[0]=c2[0];v2[1]=c2[n2-1];
	double mind=v1[0].Distance(v2[0]);
	i1=0;i2=0;
	for (int i=0;i<2;i++)
		for (int j=0;j<1;j++){
			double d=v1[i].Distance(v2[j]);
			if (d<mind){
				mind=d;
				i1=i;i2=j;
			}
